     ...
     stage('bake-image') {                 
      if (isUnix()) {
         ...
         withCredentials([[$class: 'AmazonWebServicesCredentialsBinding', accessKeyVariable: 'AWS_ACCESS_KEY_ID', credentialsId: 'AWS', secretKeyVariable: 'AWS_SECRET_ACCESS_KEY']]) { 
			ansiblePlaybook credentialsId: '14bd8691-b88a-427a-8488-cf28846b9820', installation: 'ansible', inventory: 'ansible-aws-playbooks.git/ansible.cfg', extras: '--extra-vars="env_name=$env_name"  --extra-vars="app_name=$app_name"  --extra-vars="sg_group_id=$sg_group"  --extra-vars="vpc_subnet_id=$subnet"  --extra-vars="instance_type=t2.micro" --extra-vars="ami_id=$ami_id"', playbook: 'ansible-aws-playbooks.git/provision-aws.yml', sudoUser: null
			ansiblePlaybook credentialsId: '14bd8691-b88a-427a-8488-cf28846b9820', installation: 'ansible',inventory: 'ansible-aws-playbooks.git/ansible.cfg',extras: '--extra-vars="app_version=1.0-SNAPSHOT" --extra-vars="env_name=$env_name" --extra-vars="group_id=$group_id"  --extra-vars="app_name=$app_name"', playbook: 'ansible-aws-playbooks.git/app-deploy.yml', sudoUser: null
			ansiblePlaybook credentialsId: '14bd8691-b88a-427a-8488-cf28846b9820', installation: 'ansible',inventory: 'ansible-aws-playbooks.git/ansible.cfg',extras: '--extra-vars="env_name=$env_name" --extra-vars="AWS_ACCESS_KEY_ID=$AWS_ACCESS_KEY_ID" --extra-vars="AWS_SECRET_ACCESS_KEY=$AWS_SECRET_ACCESS_KEY" --extra-vars="app_name=$app_name" --extra-vars="env_name=$env_name" --extra-vars="app_version=1.0.$BUILD_NUMBER" ', playbook: 'ansible-aws-playbooks.git/amicreate.yml', sudoUser: null
			hygieiaDeployPublishStep applicationName: "$app_name", artifactDirectory: 'petclinic.git/build/libs/',inventory: 'ansible-aws-playbooks.git/ansible.cfg', artifactGroup: 'org.springframework.samples', artifactName: '*.war', artifactVersion: '1.0.$BUILD_NUMBER', buildStatus: 'InProgress', environmentName: 'BAKE'        
		  } 
		}
    }
